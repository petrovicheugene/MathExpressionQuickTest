                                                   ...
                                                   QPair<int, bool>& highlight)
{
    ...
    for (int i = pos + 1; i < equation.count(); i++)
    {
        QChar currentSymbol = equation.at(i);
    ...
    highlight.first = equation.count() - 1;
    highlight.second = false;
}
//===================================================
...
bool ZMathExpressionHandler::zh_getToken()
{
    ...
    for (; zv_charPosition < zv_equation.count(); zv_charPosition++)
    {
        currentChar = zv_equation.at(zv_charPosition);
...
//                break;
    ...
    qDebug() << "TOKEN" << zv_token;

    zv_tokenEndPosition = zv_equation.count() - 1;
...
//}
//===================================================
void ZMathExpressionHandler::zp_defineHighLights(const QString& equation,
                                                 int cursorPos,
                                                 QVector<QPair<int, bool>>& highlightedSymbolVector)
{
    ...
    if (cursorPos < equation.count() && cursorPos >= 0)
    {
        symbol = equation.at(cursorPos);
    ...
    if (cursorPos > 0 && cursorPos <= equation.count())
    {
        symbol = equation.at(cursorPos - 1);
        for (int i = 0; i < zv_pairSymbolVector.count(); i++)
        {
...
//===================================================
bool ZMathExpressionHandler::zp_checkEquation(const QString& equation,
                                              int cursorPos,
                                              QVector<QPair<int, bool>>& highlightedSymbolVector)
{
    zp_defineHighLights(equation, cursorPos, highlightedSymbolVector);
    return true;
}
//===================================================
bool ZMathExpressionHandler::zp_calculateEquation(const QString& equation, double& dResult)
{
    zv_equation = equation;
    zv_charPosition = 0;
